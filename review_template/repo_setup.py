...
config = dict(
    ...
    GIT_ACTOR=local_config.get("general", "GIT_ACTOR", fallback=actor_fallback()),
    DEBUG_MODE=local_config.getboolean("general", "DEBUG_MODE", fallback=False),
    DATA_FORMAT=local_config.get("general", "DATA_FORMAT", fallback='["MANUSCRIPT"]'),
    PDF_HANDLING=local_config.get("general", "PDF_HANDLING", fallback="EXT"),
    ID_PATTERN=local_config.get("general", "ID_PATTERN", fallback="THREE_AUTHORS"),
...
logging.basicConfig(
    ...
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[logging.FileHandler("report.log", mode="a"), logging.StreamHandler()],
)
...
try:
    ...
    logging.error(
        f'Could not load DATA_FORMAT ({config["DATA_FORMAT"] }), ' "using fallback"
    )
