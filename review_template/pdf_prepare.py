...
def apply_ocr(record: dict) -> dict:
    ...
    command = 'docker run --rm --user "$(id -u):$(id -g)" -v \"' + \
        os.path.join(os.getcwd(), 'pdfs') + \
        ':/home/docker\" jbarlow83/ocrmypdf --force-ocr ' + options \
        + ' -l eng+deu "' + \
...
def validate_completeness(record: dict) -> dict:
    ...
    if full_version_purchase_notice in \
        ...
        logging.error(f'{record["ID"]}'.ljust(EPAD-1, ' ') +
    ...
    if nr_pages_metadata != record['pages_in_file']:
        if nr_pages_metadata == int(record['pages_in_file'])-1:
            logging.warning(f'{record["ID"]}'.ljust(EPAD-3, ' ') +
...
def mark_committed_pdfs(bib_db: BibDatabase,
...
def main(bib_db: BibDatabase,
    ...
    while in_process:
        ...
        if batch_end > 0:
            ...
            need_man_preps = [r['ID'] for r in bib_db.entries
                              if 'needs_manual_preparation' ==
                              r.get('pdf_status', 'NA')]
            if 'GIT' == repo_setup.config['PDF_HANDLING']:
