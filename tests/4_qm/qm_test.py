...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("GuyPhD, Arun", "", False),  #
        ("Rai, Arun; Straub, Detmar", "name-format-separators", True),  # incorrect delimiter
        # author without capital letters
        # NOTE: it's not a separator error, should be something more relevant
        ("Mathiassen, Lars and jonsson, katrin", "name-format-separators", True,),
        # University in author field
        ("University, Villanova and Sipior, Janice", "erroneous-term-in-field", True,),
        # Special characters
        ("Mourato, Inês and Dias, Álvaro and Pereira, Leandro", "", False,),
...
def test_get_quality_defects_author(
    ...
    if defect:
        ...
        assert defect_type in v_t_record.data['colrev_masterdata_provenance']['author']['note']
    else:
...
def test_get_quality_defects_title(
    ...
    if defect:
        assert v_t_record.has_quality_defects()
        assert defect_type in v_t_record.data['colrev_masterdata_provenance']['title']['note']
    else:
        assert not v_t_record.has_quality_defects()
