...
@pytest.mark.parametrize(
    "input_dict_1, input_dict_2, require_same_type , expected_similarity",
    [
        (
            {
                Fields.ENTRYTYPE: ENTRYTYPES.ARTICLE,
                Fields.TITLE: "TECHNOLOGICAL ENTITLEMENT: IT'S MY TECHNOLOGY AND I'LL (AB)USE IT HOW I WANT TO",
            },
            {
...
def test_get_retrieval_similarity(
    input_dict_1: dict,
    input_dict_2: dict,
    ...
    actual_similarity = colrev.record.record_prep.PrepRecord.get_retrieval_similarity(
        record=colrev.record.record.Record(input_dict_1),
        retrieved_record=colrev.record.record.Record(input_dict_2),
        same_record_type_required=require_same_type,
    )
