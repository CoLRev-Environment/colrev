    ...
    try:
        for i, row in screen.iterrows():
            if 'TODO' == row['inclusion_2']:
                ...
                print()
                print()
                print(reference['title'] + '  -  ' + reference['author'] + '  ' + reference['journal'] + '  ' + str(reference['year']) + '  (' + str(reference['volume']) + ':' + str(reference['number']) + ') ' +  reference['file'] + ' *' + reference['citation_key'] + '*')
