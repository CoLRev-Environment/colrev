...
class ScopusSearchSource(base_classes.SearchSourcePackageBaseClass):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                output_json_path = Path(
                    self.search_source.search_parameters.get('output_json', 'scopus_results.json')
                )
                output_bib_path = Path(
                    self.search_source.search_parameters.get('output_bib', 'scopus_results.bib')
                )
    ...
    def _save_simple_results(self, entries: list, json_path: Path, bib_path: Path) -> None:
        results = []
    ...
    def _load_bib(cls, *, filename: Path, logger: logging.Logger) -> dict:
        ...
        def field_mapper(record_dict: dict) -> None:
            if record_dict[Fields.ENTRYTYPE] in [ENTRYTYPES.INPROCEEDINGS, ENTRYTYPES.PROCEEDINGS]:
            ...
            if "art_number" in record_dict:
                record_dict[f"{cls.endpoint}.art_number"] = record_dict.pop("art_number")
            if "note" in record_dict:
                record_dict[f"{cls.endpoint}.note"] = record_dict.pop("note")
            if "document_type" in record_dict:
                record_dict[f"{cls.endpoint}.document_type"] = record_dict.pop("document_type")
