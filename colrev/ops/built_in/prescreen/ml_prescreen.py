...
class MlPrescreen(JsonSchemaMixin):
        ...
        for entry in bibtex_entries.split("\n\n"):
            ...
            if colrev_status.startswith("{rev_"):
                ...
                data["abstract"].append(
                    self.preprocess_text(bib_dict.get("abstract", ""))
                )
                data["label_included"].append(
                    1
                    if "{rev_synthesized}" in colrev_status
                    or "{rev_included}" in colrev_status
                    else 0
                )
    ...
    def train_model(self, df):
        documents = [
            TaggedDocument(
                words=word_tokenize(str(title) + " " + str(abstract)), tags=[str(i)]
            )
            for i, (title, abstract, _) in enumerate(
                zip(df["title"], df["abstract"], df["label_included"])
            )
        ]
        model = Doc2Vec(
            vector_size=300, window=5, min_count=4, dm=0, epochs=30, seed=42
        )
    ...
    def create_and_predict_dataframe(self, bibtex_file):
        ...
        with open(bibtex_file, encoding="utf-8") as file:
            bibtex_entries = file.read()
        entries = [
            entry.strip() for entry in bibtex_entries.split("\n\n") if entry.strip()
        ]
    ...
    def _ml_prescreen(self) -> bool:
        ...
        knn_classifier = KNeighborsClassifier(
            n_neighbors=2, weights="uniform", metric="euclidean"
        )
        ...
        for key, value in prediction_data.items():
            ...
            if colrev_origin is not None:
                cleaned_prediction_key = [
                    item.strip().rstrip(",")
                    for item in colrev_origin.replace("{", "")
                    .replace("}", "")
                    .split(";")
