...
import semanticscholarvalidator
...
class SemanticScholarSearchSource(JsonSchemaMixin):
    ...
    validator = semanticscholarvalidator.Validator()
        ...
        for key, value in params.items():
            match key:
                ...
                case "year":
                    ...
                    year_ok = False
                    while not year_ok:
                        if self.validator.is_valid_yearspan(value):
                            year_ok = True
                        else:
                            year = self.__s2_UI__.enter_year(retry=True)
    ...
    def __get_api_key(self) -> str:
        ...
        if not api_key: 
            api_key = self.__s2_UI__.get_api_key(retry=False)

            if self.validator.is_valid_api_key(api_key=api_key):
                self.review_manager.environment_manager.update_registry(self.SETTINGS["api_key"], api_key)
            else:
                api_key = self.__s2_UI__.get_api_key(retry=True)
                self.review_manager.environment_manager.update_registry(self.SETTINGS["api_key"], api_key)
